...
const Module *Executor::setModule(llvm::Module *module, 
  ...
  kmodule->prepare(opts, interpreterHandler, ra, aa, mra);
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Load: {
    ...
    if (state.isNormalState() && state.getRecoveryState() != 0) {
      if (isBlockingLoad(i)) {
  ...
  if (success) {
    ...
    if (inBounds) {
      ...
      if (isWrite) {
        ...
        onObjectStateRead(state, mo, offset, type);
...
bool Executor::isBlockingLoad(Instruction *load_inst) {
  ModRefAnalysis::LoadToStoreMap::iterator entry = mra->loadToStoreMap.find(load_inst);
  return entry != mra->loadToStoreMap.end();
}
...
void Executor::startRecoveryState(ExecutionState &state, RecoveryInfo *recoveryInfo) {
  /* get the corresponding recovery state */
  ExecutionState *recoveryState = state.getRecoveryState();
...
void Executor::onObjectStateRead(ExecutionState &state, const MemoryObject *mo, ref<Expr> offset, Expr::Width width) {
  klee_message("read in state %p: mo = %p, address = %llx", state, mo, mo->address);
  offset->dump();
}
